...
class MolNet(InMemoryDataset):
    def __init__(self, root='dataset', dataset=None, xd=None, y=None, transform=None, pre_transform=None, smile_graph=None,
        ...
        if os.path.isfile(self.processed_paths[0]):
            ...
            self.process(xd, y, smile_graph,fp1_list, fp2_list, fp3_list, fp4_list)
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self, xd, y, smile_graph,fp1_list, fp2_list, fp3_list, fp4_list):
        ...
        for i in range(data_len):
            ...
            if smiles is not None:
                ...
                fp1 = fp1_list[i].squeeze()  # 确保形状为 [881]
                fp2 = fp2_list[i].squeeze()  # 确保形状为 [1024]
                fp3 = fp3_list[i].squeeze()  # 确保形状为 [780]
                fp4 = fp4_list[i].squeeze()  # 确保形状为 [1024]

                # 转换为二维张量 [1, feature_dim]
                ...
                electronic_desc_tensor = torch.nan_to_num(electronic_desc_tensor, nan=0.0)  # 关键修复

                qm9_tensor = torch.FloatTensor(qm9_feat).view(1, -1)
                qm9_tensor = torch.nan_to_num(qm9_tensor, nan=0.0)  # 关键修复
